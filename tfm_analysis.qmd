...
df_pca <-
  ...
  select(-c(CPRO, median_inc_con_unit, diff_ue, per_capita_CCAA, dist_prov_cap, n_firms, per_capita_n_firms, per_capita_MUN)) |> 
  drop_na()
...
df_clus <-
  df_dea |> 
  select_if(is.numeric) |> 
  select(-c(CPRO, diff_ue, median_inc_con_unit, per_capita_CCAA, per_capita_MUN)) |> 
  drop_na()
```
...
df_model <- 
  df_dea |> 
  select(-c(CODAUTO, dist_prov_cap, per_capita_CCAA, per_capita_MUN, per_capita_n_firms, median_inc_con_unit, party_mun, party_ccaa, diff_ue, n_firms))
```
...
df_mae_lm <-
  na.omit(df_mae) |> 
  select(-c(INE, NOMBRE, CCAA, party_ccaa, party_mun, median_inc_con_unit, diff_ue, dea_robust))
...
rf_tune_final <- train(dea_robust ~., 
                 ...
                 tuneGrid = data.frame(mtry= c(45:53)), #best tunes found in this range 
                 importance = TRUE)
```
