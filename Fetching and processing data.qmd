...
```{r}
contracts <- 
  read_xlsx("Data/Contratos_por_municipios_2022.xlsx", skip = 1) |> 
...
# Combine all data frames into a single object
social_security_2022 <- bind_rows(all_data)
```
...
unemployment_2022 <- 
  ...
  summarise(mean_ue = mean(Paro_total)) |> 
  ungroup() |> 
  left_join(social_security_2022, by = "INE") |> 
  select(INE, NOMBRE, mean_ue, mean_ss) |> 
  mutate(ue_pct_2022 = (mean_ue/(mean_ue + mean_ss))*100) |> 
  drop_na()
```
...
df <- 
  ...
  left_join(unemployment_2022, by = "INE") |>
