...
## Contracts (2023 - change back if necessary)
...
social_security <- 
  ...
  drop_na(TOTAL_SS) |> 
  group_by(INE) |> 
  summarise(mean_ss_2022 = mean(TOTAL_SS, rm.na = T)) |> 
  ungroup()
...
unemployment <- 
  ...
  select(INE, NOMBRE, mean_ue, mean_ss_2022) |> 
  mutate(ue_pct_2022 = (mean_ue/(mean_ue + mean_ss_2022))*100) |> 
  drop_na()
```

```{r}
unemployment |> 
  summarise(ue = sum(mean_ue), ss = sum(mean_ss_2022)) |>
  mutate(ue_pct_2022 = (ue/(ue + ss))*100)
```
...
unemployment <- 
  unemployment |> 
  select(INE, ue_pct_2022, mean_ss_2022)
```

## Change in unemployment

Idea change over month as change in number of unemployed to calculate job creation later on

```{r}
ue_2022 <- read_xlsx("Data/Paro_por_municipios_segundo_semestre_2022.xlsx", skip = 1) |> 
  select("mes", "Codigo Municipio", "Municipio", "total Paro Registrado") |> 
  filter(mes == "Diciembre de 2022") |> 
  rename(INE = "Codigo Municipio",
         NOMBRE = Municipio,
         Paro_total = "total Paro Registrado") |> 
  mutate(INE = ifelse(nchar(INE) == 4, paste0("0", INE), INE)) 
...
ue_change <- rbind(ue_2022, ue_2023)

rm(ue_2022, ue_2023)

ue_change <- 
  ue_change |> # 2.5 because in between 0 and 5
  mutate(Paro_total = ifelse(Paro_total == "<5", 2.5, as.numeric(Paro_total))) |>  
  group_by(INE) |> 
  mutate(ue_change = Paro_total - lag(Paro_total)) |>
  ungroup() |> 
  filter(mes != "Diciembre de 2022") |> 
  group_by(INE) |> 
  summarise(mean_ue_change = mean(ue_change, rm.na = T),
            mean_ue_2023 = mean(Paro_total, rm.na = T)) |> 
  ungroup()
```

Social security for score generation down below
...
ss_2023 <- 
  ...
  drop_na(TOTAL_SS) |> 
  group_by(INE) |> 
  summarise(mean_ss_2023 = mean(TOTAL_SS, rm.na = T)) |> 
  ungroup()
```
...
df <- 
  ...
  left_join(ue_change, by = "INE") |>  
  left_join(ss_2023, by = "INE") |>
...
df_final <- 
  df_final |> 
  filter(!is.na(ue_pct))
#per_capita_contracts = contracts_mean_2023/pop_total,
```
...
df_final <- 
  df |> 
  mutate(job_creation = ((contracts_mean_2023 / (mean_ss_2023 + mean_ue_2023)) - (mean_ue_change / (mean_ss_2023 + mean_ue_2023))) * 100) |> 
  drop_na(job_creation)
