...
Reading CCAA 2022 population data to calculate spending per inhabitant Data from: https://econet.carm.es/inicio/-/crem/sicrem/PU_padron/cifof10/sec2_c1.html
...
contracts <- 
  ...
  group_by(INE) |> 
  summarise(contracts_mean_2022 = mean(Contratos_total))
```
...
```
```{r, eval=FALSE}
write.csv(avg_age_final, "Data/avg_age_final.csv")
```

```{r}
avg_age_final <- read.csv("Data/avg_age_final.csv", colClasses = c(INE = "character")) |> 
  select (-X)
```
...
social_security_2023 <- do.call(rbind, all_data)
...
social_security_2023 <- 
  ...
  drop_na(TOTAL_SS) |> 
  mutate(mes = case_when(mes == 1 ~ "Enero de 2023",
                         mes == 2 ~ "Febrero de 2023",
                         mes == 3 ~ "Marzo de 2023",
                         mes == 4 ~ "Abril de 2023",
                         mes == 5 ~ "Mayo de 2023",
                         mes == 6 ~ "Junio de 2023",
                         mes == 7 ~ "Julio de 2023",
                         mes == 8 ~ "Agosto de 2023",
                         mes == 9 ~ "Septiembre de 2023",
                         mes == 10 ~ "Octubre de 2023",
                         mes == 11 ~ "Noviembre de 2023",
                         mes == 12 ~ "Diciembre de 2023"))
```


```{r}
unemployment_2023_monthly <- 
  unemployment_2023 |> 
  left_join(social_security_2023, by = c("INE", "mes")) |> 
  mutate(ue_mes_2023 = (Paro_total / (Paro_total + TOTAL_SS))*100) |> 
  select(mes, INE, ue_mes_2023)|> 
...
```{r}
unemployment_2022 <- 
  read_xlsx("Data/Paro_por_municipios_2022.xlsx", skip = 1) |> 
  select("mes", "Codigo Municipio", "Municipio", "total Paro Registrado") |> 
  rename(INE = "Codigo Municipio",
         NOMBRE = Municipio,
         Paro_total = "total Paro Registrado") |> 
...
# Combine all data frames into one if needed
social_security_2022 <- do.call(rbind, all_data)

social_security_2022 <- 
  social_security_2022 |> 
  mutate(MUNICIPIO = str_replace(MUNICIPIO, "^(\\S+) ", "\\1_")) |> 
  separate(MUNICIPIO, into = c("INE", "NOMBRE"), sep = "_") |> 
  mutate(TOTAL_SS = gsub("\\.", "", TOTAL)) |> # deleting dots
  ...
  select(INE, TOTAL_SS, mes) |> 
  drop_na(TOTAL_SS) |> 
  mutate(mes = case_when(mes == 1 ~ "Enero de 2022",
                         mes == 2 ~ "Febrero de 2022",
                         mes == 3 ~ "Marzo de 2022",
                         mes == 4 ~ "Abril de 2022",
                         mes == 5 ~ "Mayo de 2022",
                         mes == 6 ~ "Junio de 2022",
                         mes == 7 ~ "Julio de 2022",
                         mes == 8 ~ "Agosto de 2022",
                         mes == 9 ~ "Septiembre de 2022",
                         mes == 10 ~ "Octubre de 2022",
                         mes == 11 ~ "Noviembre de 2022",
                         mes == 12 ~ "Diciembre de 2022"))
```


```{r}
unemployment_2022_monthly <- 
  unemployment_2022 |> 
  left_join(social_security_2022, by = c("INE", "mes")) |> 
  mutate(ue_mes_2022 = (Paro_total / (Paro_total + TOTAL_SS))*100) |> 
  select(mes, INE, ue_mes_2022) |> 
  mutate(mes = str_extract(mes, "^\\w+"))
```
...
target <- 
  unemployment_2023_monthly |> 
  left_join(unemployment_2022_monthly, by = c("INE", "mes")) |> 
  mutate(target = ue_mes_2023 - ue_mes_2022) |> 
  group_by(INE) |> 
...
social_security_2023 <- bind_rows(all_data)
```
...
unemployment_2023 <- 
  ...
  ungroup() |> 
  left_join(social_security_2023, by = "INE") |> 
  select(INE, NOMBRE, mean_ue, mean_ss) |> 
  mutate(ue_pct_2023 = (mean_ue/(mean_ue + mean_ss))*100) |> 
  drop_na()
```
...
df <- 
  ...
  left_join(unemployment_2023, by = "INE") |> 
...
df <- 
  ...
  mutate(per_capita_MUN = total_MUN/pop_total,
         ...
         per_capita_contracts = contracts_mean_2022/pop_total,
...
df <- 
  ...
  mutate(per_capita_CCAA = if_else(CCAA == "Ceuta", per_capita_MUN, per_capita_CCAA)) |> # Ceuta has the same budget on the CCAA as municipal level
  mutate(total_CCAA = if_else(CCAA == "Ceuta", total_MUN, total_MUN))
```
...
```{r}
write.csv(df_final, paste("Data/TFM_data_", Sys.Date(), ".csv", sep = ""))
```
