...
tb_cuentasProgramas <- as_tibble(tb_cuentasProgramas) 
```
...
df_spending_mun_tidy <-
  tb_partedeImporte |> 
  select(INE, NombreEnte, CODAUTO, CPRO, CMUN, DC, NOMBRE, nombre, share_spending) |> 
...
```{r}
n_distinct(df_spending_mun_tidy$INE)
```
