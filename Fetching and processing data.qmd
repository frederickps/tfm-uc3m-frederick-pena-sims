...
--> slightly deflated unemployment numbers expected

--\> <https://datosmacro.expansion.com/paro/espana/municipios>

Idea: Using information of province activity rates to estimate the number of active population by municipality

see here: https://www.ine.es/jaxiT3/Tabla.htm?t=3996&L=0

If not doing that --> unemployment rate: 7.466369% -- 4% lower
If doing that --> unemployment rate: 13.06439% -- 2% higher
...
Joining activity rate by province and population stats

--> then

```{r}
participation <- 
  read_excel("Data/active_pop_prov.xls", skip = 8) |> 
  rename(Provincia = ...1,
         T4 = "Tasa de actividad...2",
         T3 = "Tasa de actividad...3",
         T2 = "Tasa de actividad...4",
         T1 = "Tasa de actividad...5") |> 
  mutate(Provincia = str_replace(Provincia, "^(\\S+) ", "\\1_")) |> 
  separate(Provincia, into = c("CPRO", "Provincia"), sep = "_") |> 
  drop_na(T4) %>%
  mutate(part_2023 = rowMeans(select(., starts_with("T")))) |> 
  select(CPRO, part_2023)

paro_final <- 
  paro |> 
  group_by(INE, NOMBRE) |> 
  summarise(mean_paro = mean(Paro_total, na.rm = T)) |> 
  ungroup() |> 
  left_join(codigo_municipio, by = "INE") |> 
  rename(NOMBRE = NOMBRE.x) |> 
  select(INE, NOMBRE, mean_paro, CPRO) |> 
  left_join(participation, by = "CPRO") |> 
  left_join(population, by = "INE") |> 
  select(-NOMBRE.y) |> 
  rename(NOMBRE = NOMBRE.x) |> 
  rename(wrk_age = "De 16 a 64 años total") |> 
  select(INE, NOMBRE, mean_paro, CPRO, part_2023, wrk_age) |> 
  mutate(pop_part = (part_2023 / 100) * wrk_age,
         ue_pct = (mean_paro / pop_part)*100) |> 
  drop_na(ue_pct) |> 
  select(INE, NOMBRE, ue_pct)

paro_final |> 
  summarise(l = mean(ue_pct, rm.na = T))
```
...
paro_final <- 
  ...
  rename(NOMBRE = NOMBRE.x) |> 
  rename(wrk_age = "De 16 a 64 años total") |> 
  mutate(ue_pct = (mean_paro/wrk_age)*100) |> 
  drop_na() |> 
  select(INE, NOMBRE, ue_pct)
...
paro_final |> 
  summarise(l = mean(ue_pct, rm.na = T))
```
4% difference for yearly average
