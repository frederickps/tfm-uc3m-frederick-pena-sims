...
```{r}
df_final <- read.csv("Data_2020/TFM_data_2024-05-30.csv", colClasses = c(INE = "character")) |> 
  select(-X)
```
...
df_model <- 
  df_dea |> 
  select(-c(INE, NOMBRE, CCAA, CODAUTO, CPRO, total_CCAA, total_MUN, target, dist_prov_cap, per_capita_CCAA, per_capita_MUN, n_firms, contracts_mean_2020, median_inc_con_unit))
```
...
df_model_final <- 
  df_model_robust |> 
  dplyr::select(all_of(non_zero_vars), dea_robust, greater_20k, party_mun, party_ccaa)
