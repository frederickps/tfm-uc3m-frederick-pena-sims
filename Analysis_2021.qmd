...
```{r}
ggsave("Data_2020/Figures/Figure_2.png", dpi = 300, width = 10, height = 7, bg = "white")
```
...
```
```{r}
ggsave("Data_2020/Figures/Figure_3.png", bg ="white", width = 10, height = 7)
```
...
```{r}
ggsave("Data_2020/Figures/Figure_4.png", bg = "white", dpi = 300, width = 10, height = 7)
```
...
```
```{r}
ggsave("Data_2020/Figures/Figure_5.png", dpi = 300, width = 10, height = 7, bg = "white")
```
...
rf_tune <- train(dea_robust ~., 
                 ...
                 trControl = ctrl,
                 ntree = 100,
                 tuneGrid = data.frame(mtry=c(15,16,17,18, 19, 20)),
                 importance = TRUE)
...
```{r}
ggsave("Data_2020/Figures/Figure_6.png", bg = "white", width = 10, height = 7, dpi = 300)
```
...
```
```{r}
ggsave("Data_2020/Figures/Figure_7.png", bg = "white", width = 10, height = 7, dpi = 300)
```
...
```{r}
ggsave("Data_2020/Figures/Figure_8.png", bg = "white", dpi = 300, width = 10, height = 7)
```
...
# DELETE
```{r}
variables_to_plot <- c("general_services", "employment_promotion", "community_wellbeing", "environment", "social_services")

# Create boxplots
for (variable in variables_to_plot) {
  p <- ggplot(df_achievement, aes_string(x = "o_u_achiever", y = variable)) +
    geom_boxplot() +
    labs(title = paste("Boxplot of", variable, "by Achiever Category"),
         x = "Achiever Category",
         y = variable) +
    theme_minimal()
...
```
Contextualize (MAE in the beginning)
```{r}
df_achievement |> 
  group_by(CPRO, o_u_achiever) %>%
  summarise(count = n()) %>%
  mutate(share = count / sum(count) * 100) %>%
  ungroup() |> 
  complete(CPRO, o_u_achiever, fill = list(count = 0, share = 0)) |> 
  arrange(o_u_achiever, share) |> 
  filter(o_u_achiever != "as predicted") |> 
  ggplot(aes(x = CPRO, y = share, fill = o_u_achiever)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.8)+ 
  labs(x = "Province Code",
       y = "Share (%)",
       fill = "o_u_achiever") +
  theme_minimal() +
...
```{r}
ggsave("Data_2020/Figures/Figure_9.png", bg = "white", width = 10, height = 7, dpi = 300)
```
...
```{r}
Figure_9 <- p1 + p2 + p3 + p4

Figure_9
```

```{r}
ggsave("Data_2020/Figures/Figure_9.png", width = 10, height = 7, dpi = 300, bg = "white")
```

ADD to Appendix

General Services and Pop Total
...
```{r}
Figure_10 <- p5 + p6 + p7 + p8

Figure_10
```

```{r}
ggsave("Data_2020/Figures/Figure_10.png", width = 10, height = 7, dpi = 300, bg = "white")
```
...
Figure_11 <- wrap_plots(p9, p10, p11, ncol = 1)

Figure_11
```

```{r}
ggsave("Data_2020/Figures/Figure_11.png", width = 10, height = 12, dpi = 300, bg = "white")
```

#### Dependence plots - Provinces
...
```{r}
ggsave("Data_2020/Figures/Figure_12.png", dpi = 300, width = 12, height = 7, bg = "white")
```
...
Figure_13 <- p12 + p13 + p14 + p15

Figure_13
```

```{r}
ggsave("Data_2020/Figures/Figure_13.png", width = 14, height = 10, dpi = 300, bg = "white")
```
...
```{r}
ggsave("Data_2020/Figures/Figure_1.png", dpi = 300, bg = "white", width = 10, height = 7)
```
