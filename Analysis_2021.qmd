...
```{r}
df_final <- read.csv("Data_2020/TFM_data_2024-06-12.csv", colClasses = c(INE = "character", greater_20k = "factor")) |> 
  select(-X)
```
...
df_dea |> 
  ggplot()+
  geom_histogram(aes(dea_robust), stat = "bin", bins = 100) +
  theme_minimal() +
...
ggplot(predictions, aes(x=fit, y=real))+
  ...
  labs(x = "Predicted Efficiency",y="Efficiency")
```
title = "Under- and Overachievers - Prediction Intervals", 
...
ggplot(partial_plot_grants, aes(x = revenue_grants_share_3, y = yhat)) +
  geom_line(color = "steelblue") +
  labs(title = "Partial Dependence Plot for Share of Revenue from Grants",
       x = "Share of Revenue from Grants (^3)",
       y = "Predicted Efficiency Score") +
  theme_minimal() +
  theme(axis.ticks = element_blank()) 
```
...
sv_dependence(sv, v = SHAP_structural)
```
...
table1(~ . - INE - CCAA - CPRO - NOMBRE, data = df_dea)
```
