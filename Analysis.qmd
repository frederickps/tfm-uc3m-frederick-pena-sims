...
### Naive
```{r}
X <- df_final[c("n_firms", "per_capita_MUN", "per_capita_CCAA", "dist_prov_cap", "per_capita_contracts")]

Y <- df_final[c("target", "ue_pct_2023")]
Y <- 
  Y |> 
  mutate(ue_pct_2023 = 100 - ue_pct_2023) # DEA does not know how to handel negative value --> adding 100 to 
...
### Robust
```{r}
X <- df_final[c("n_firms", "per_capita_MUN", "per_capita_contracts")]

Y <- df_final[c("target")]
Y <- 
  Y |> 
  mutate(ue_pct_2023 = 100 - ue_pct_2023) # DEA does not know how to handle negative value --> adding 100 to 
...
df_model <- 
  df_dea |> 
  select(-c(INE, NOMBRE, CCAA, CODAUTO, CPRO, total_CCAA, total_MUN, pop_below_16, pop_16_64, pop_above_65, pop_foreigners, target, dist_prov_cap, per_capita_CCAA, per_capita_MUN, n_firms, ue_pct_2023, pop_share_wk_age, contracts_mean_2022, per_capita_contracts))
```
...
df_model_final <- 
  df_model_imp |> 
  dplyr::select(all_of(non_zero_vars), party_mun, party_ccaa, dea_naive, greater_20k)
```
