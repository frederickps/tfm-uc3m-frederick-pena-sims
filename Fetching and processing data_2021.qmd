...
Importer includes the recognized rights, which in this context means the revenues that have been formally acknowledged by the municipality for the given fiscal year. This metric will provide you with the most accurate figure for the total revenue that the municipality has recognized during that year, encompassing all sources of revenue that have been recorded as rights.
...
share calculation will be done at the end when merging with population data (share because Cifuentes + municipal budget balancing)

to see how and why summarized see Cifuentes tables
...
revenue_municipality_final <- 
  ...
  mutate(revenue_tax_share = revenue_tax/total_revenue_MUN,
         ...
         per_capita_revenue = total_revenue_MUN/total_revenue_MUN) |> 
  select(INE, revenue_tax_share, revenue_grants_share, revenue_other_share, per_capita_revenue)
```
...
repetetive procedure --\> hence function

```{r}
spending_CCAA <- 
  function(data_file, sheet, CCAA){
  data <-
...
ccaa_general_services, ccaa_security, ccaa_economic, ccaa_housing_services, ccaa_health, ccaa_culture, ccaa_social_protection \

## Contracts

share temporary of all contracts

share service sector of all contracts
...
contracts <- 
  ...
  mutate(Contratos_total = ifelse(Contratos_total == "<5", 2.5, as.numeric(Contratos_total)),
         share_contracts_agri = (Contratos_agri/Contratos_total)*100,
         share_contracts_indu = (Contratos_indu/Contratos_total)*100,
         share_contracts_con = (Contratos_cons/Contratos_total)*100,
         share_contracts_serv = (Contratos_serv/Contratos_total)*100) |> 
  group_by(INE) |> 
...
Different municipalities have different required expenditures

More variables (How to make a variable out of it?) 0 – 5,0000 5,000 – 20,000 20,000 – 50,000 50,000 and above
...
-   saving in data frame because it takes too long
...
share of firms in Comercio, transporte y hostelería
...
n_firms  <- 
  ...
  mutate(n_firms = as.numeric(n_firms),
         ...
         share_firms_service = (n_firms_service/n_firms)*100) |> 
  separate(municipio, into = c("INE", "NOMBRE"), sep = "_") |> 
...
share of sector specific unemployed of all unemployed
...
unemployment_2020 <- 
  ...
  mutate(INE = ifelse(nchar(INE) == 4, paste0("0", INE), INE)) |> # 2.5 because in between 0 and 5
  mutate(Paro_total = ifelse(Paro_total == "<5", 2.5, as.numeric(Paro_total)),
         share_ue_service = (Paro_servicio/Paro_total)*100,
         share_ue_construction = (Paro_constr/Paro_total)*100,
         share_ue_industry = (Paro_indu/Paro_total)*100,
         share_ue_agriculture = (Paro_agri/Paro_total)*100,
         share_ue_wo_job = (Paro_sin_emp/Paro_total)*100) |> 
...
Adding information of share of businesses providing essential services in covid pandemic 

Adding information on share of employment providing essential services in Covid pandemic

Data: https://www.ine.es/jaxi/Tabla.htm?path=/COVID/dirce/l0/&file=01002.px&L=0

Royal Decree: https://www.ine.es/covid/nota_tecnica_dirce.pdf
...
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC10864869/

https://zenodo.org/records/10066112
...
Merging with municipal and CCAA data
...
sum(is.na(political_leadership_mun$party_mun_id))
```
1968 NAs

Score will be crated when dfs are merged
...
## REMOVE DEA List of useful dataframes
...
### Adjusting budget of Ceuta and Melilla
